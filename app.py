...
def chat_with_pdf(prompt, pdf_path):
    with open(pdf_path, "rb") as file_obj:
        file_upload = openai.files.create(file=file_obj, purpose="assistants")
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {"type": "file", "file_id": file_upload.id}
                ]
            }
        ],
        max_tokens=1500
...
def upload():
    try:
        ...
        # === PROMPTS ===
        table_prompt = f"""
You are generating a professional car listing HTML table in the Car Cafe format.
The table should have 3 columns: Details, Information, Options.
Use orange border styles, bold font, center-aligned cells. Example structure:

<tr>
  <td style="...">Year</td>
  <td style="...">2015</td>
  <td style="...">Leather, Backup Cam</td>
</tr>
...
Write a professional Car Cafe description for this car based on the Carfax PDF and info provided. 
Use a warm, clean tone. Focus on service records, condition, features, and tires.
...
"""
        ...
        # === Build HTML
        gallery_html = "\n".join([
            f'<img src="{url}" alt="Image {i+1:03d}" style="width: 100%; max-width: 500px; height: auto; border-radius: 5px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">'
...
<div style="font-family: Arial;">
  ...
  {description_html}
  <div style='text-align:center; margin: 30px 0;'>{video_html}</div>
  <h2 style='text-align: center; font-size: 28px;'>Gallery</h2>
  <div style='display: grid; grid-template-columns: 1fr 1fr; gap: 20px;'>{gallery_html}</div>
